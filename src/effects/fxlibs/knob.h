...
class knob
{
  public:
    ...
    */
    void Init(Curve curve = knob::LINEAR, float min = 0.f, float max = 1.f);


    void UnityCalibrate(float offset = 0.f, float scale = 1.f, float zero_lim = 0.f,float one_lim = 1.f);



    float UnityProcess(float adcfloat);
  ...
  private:
    ...
    float         unity_offset_;
    float         unity_scale_ ; 
    float         unity_zero_lim_;
    float         unity_one_lim_; 
    Curve         pcurve_;
};
